...
class CrabNetLightning(L.LightningModule):
    def __init__(self, **config):
        ...
        y=np.array([train_data['form_energy_per_atom'].values, train_data['energy_above_hull'].values, train_data['stable'].values,
           train_data['disorder'].values]).T # flag
        
        self.step_size = len(train_data) # flag

        self.scaler=CombinedScaler(y,scale_map=[True,True,False,False])
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.criterion_energy(pred[:,0].view(-1),
                              unc[:,0].view(-1),
                              y[:,0].view(-1))+\
        self.criterion_hull(pred[:,1].view(-1),
                              unc[:,1].view(-1),
                              y[:,1].view(-1))+\
        self.criterion_stability(pred[:,2].view(-1),
                              unc[:,2].view(-1),
                              y[:,2].view(-1))+\
        self.criterion_disorder(pred[:,3].view(-1),
                              unc[:,3].view(-1),
                              y[:,3].view(-1))         
        ...
        mse_en = masked_mse(pred[:,0].view(-1),y[:,0].view(-1))
        mae_en = masked_mae(pred[:,0].view(-1),y[:,0].view(-1))
        mse_hull = masked_mse(pred[:,1].view(-1),y[:,1].view(-1))
        mae_hull = masked_mae(pred[:,1].view(-1),y[:,1].view(-1))
        acc_stab = masked_acc(pred[:,2].view(-1),y[:,2].view(-1))
        acc_dis = masked_acc(pred[:,3].view(-1),y[:,3].view(-1))
        self.log("train_mse_en", mse_en, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        self.log("train_mae_en", mae_en, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        self.log("train_mse_hull", mse_hull, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        self.log("train_mae_hull", mae_hull, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        self.log("train_acc_stab", acc_stab, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        self.log("train_acc_dis", acc_dis, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        acc_stab = masked_acc(pred[:,2].view(-1),y[:,2].view(-1))
        acc_dis = masked_acc(pred[:,3].view(-1),y[:,3].view(-1))
        self.log("val_mse_en", mse_en, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        self.log("val_mae_en", mae_en, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        self.log("val_mse_hull", mse_hull, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        self.log("val_mae_hull", mae_hull, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        self.log("val_acc_stab", acc_stab, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        self.log("val_acc_dis", acc_dis, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        return val_loss
    ...
    def test_step(self, batch, batch_idx):
        ...
        acc_stab = masked_acc(pred[:,2].view(-1),y[:,2].view(-1))
        acc_dis = masked_acc(pred[:,3].view(-1),y[:,3].view(-1))
        self.log("test_mse_en", mse_en, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        self.log("test_mae_en", mae_en, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        self.log("test_mse_hull", mse_hull, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        self.log("test_mae_hull", mae_hull, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        self.log("test_acc_stab", acc_stab, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        self.log("test_acc_dis", acc_dis, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        return 
...
if __name__=='__main__':
    ...
    config={'train_path': 'data/materials_data/all_properties/train_total.csv',
            'val_path': 'data/materials_data/all_properties/val_total.csv',
            'test_path': 'data/materials_data/all_properties/test_total.csv',
            'out_dims': 3,
            'd_model': 512,
            'N': 3,
            'heads': 4,
    ...
    trainer = Trainer(max_epochs=200, accelerator="gpu", devices=1, logger=wandb_logger,
